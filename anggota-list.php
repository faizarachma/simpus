<div class="p-3">
	...
	<table class="table table-striped">
		...
		if($_SERVER['REQUEST_METHOD'] == "POST"){
			...
			if($pencarian != ''){
				$query = mysqli_query($koneksi, "SELECT * FROM tbanggota WHERE
					kdanggota LIKE '%$pencarian%'
				...
				$query = mysqli_query($koneksi, "SELECT * FROM tbanggota lIMIT $posisi, $batas");
				$query_jml = mysqli_query($koneksi, "SELECT * FROM tbanggota");
				$no = $posisi + 1;
				}
		}
		else{
		...
		foreach($query as $ky => $val){
			...
			<tr>
				...
				<td class="text-right">
					<div class="btn-group" role="group" aria-label="Basic example">
						<a href="anggota-print.php?id=<?=$val['idanggota']?>" class="btn btn-sm btn-info" target="_blank">
							<i class="fa fa-address-card"></i> Cetak Kartu
						</a>
						<a href="index.php?page=anggota-form&mode=edit&id=<?=$val['idanggota']?>" class="btn btn-sm btn-success">
							<i class="fa fa-edit"></i> Edit
						</a>
						<a href="anggota-aksi.php?mode=delete&id=<?=$val['idanggota']?>" class="btn btn-sm btn-danger" onclick="return confirm('Apakah anda yakin akan menghapus <?=$nama_kode ?>')">
